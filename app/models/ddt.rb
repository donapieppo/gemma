class Ddt < ApplicationRecord
  ...
  def no_other_in_year
    ...
    if ( other > 1 ) or ( self.new_record? and other > 0 ) 
  ...
  def no_previous_loads
    if ! self.new_record? and self.operations.where('date < ?', self.date).count > 0
  ...
  def myvalidate
    # FIMXE 
    # non controlliamo organization_id che viene settata nel controller
  ...
  def set_new_number
    ...
    if last
      self.number = last.number.to_i + 1 
    else
      self.number = 1
    end
  end

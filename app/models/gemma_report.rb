...
class GemmaReport
    ...
    when "csv"
      ...
      CSV.generate(col_sep: ";", quote_char: '"') do |csv|
        ...
        ActiveRecord::Base.connection.execute(@query).each(as: :hash) do |record|
          if @fields.include?(:price_operations) && record["price_operations"]
  ...
  def pdf_body
    ...
    ActiveRecord::Base.connection.execute(@query).each(as: :hash) do |record|
      ...
      if @separator && old != record[@separator.to_s]
        print_last_line
      ...
      if @accumulator && record[@accumulator.to_s]
        @accumulator_value += record[@accumulator.to_s].to_f
      end
      if @price_accumulator && record["price"]
      ...
      if @fields.include?(:type)
        if record["type"] == "Stock" || record["type"] == "ArchStock"
          record["upn"] = "Giacenza iniziale"
        elsif record["type"] == "Load" || record["type"] == "ArchLoad"
      ...
      line = @fields.inject(@initial_space) do |tot, f|
        ...
        if record[f.to_s] && (f != :type) && !((f == :price) && (record["price"] == 0.0))
  ...
  def print_last_line
    ...
    if @price_accumulator && @price_accumulator_value > 0
      @pdf.move_down 10

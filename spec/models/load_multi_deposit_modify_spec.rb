...
describe Load do
  ...
  it "should be possible to change load number so that unload becomes 0" do
    FactoryBot.create(:unload, numbers: {@deposits[1].id => 1 - @num[1]}, thing: @thing, date: @now + 5.days)
  ...
  it "should not be possible to change load date so that one deposit actual becomes negative" do
    FactoryBot.create(:unload, numbers: {@deposits[1].id => 1 - @num[1]}, thing: @thing, date: @now)
